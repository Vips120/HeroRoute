"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("@angular/core");
var ZippyComponent = (function () {
    function ZippyComponent() {
        this.title = {
            zippy1: "Who Can Contact me",
            zippy2: "Who Can See me"
        };
        this.ZippChange = false;
        this.ZippyPanda = false;
        this.ZippyPoto = true;
    }
    ZippyComponent.prototype.ZippyControl = function () {
        this.ZippChange = !this.ZippChange;
    };
    ZippyComponent.prototype.ZippyTanda = function () {
        this.ZippyPanda = !this.ZippyPanda;
    };
    ZippyComponent.prototype.CntrlPanel = function () {
        this.ZippyPoto = !this.ZippyPoto;
    };
    __decorate([
        core_1.Input(), 
        __metadata('design:type', Object)
    ], ZippyComponent.prototype, "ZippChange", void 0);
    __decorate([
        core_1.Input(), 
        __metadata('design:type', Object)
    ], ZippyComponent.prototype, "ZippyPanda", void 0);
    __decorate([
        core_1.Input(), 
        __metadata('design:type', Object)
    ], ZippyComponent.prototype, "ZippyPoto", void 0);
    ZippyComponent = __decorate([
        core_1.Component({
            selector: 'zippy',
            templateUrl: "./app/zippy_component.html",
            styleUrls: ['./app/zippy_component.css']
        }), 
        __metadata('design:paramtypes', [])
    ], ZippyComponent);
    return ZippyComponent;
}());
exports.ZippyComponent = ZippyComponent;
//# sourceMappingURL=zippy.component.js.map